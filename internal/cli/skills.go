...
func ensureSkills() ([]string, error) {
	...
	skills := []struct {
		...
		{
			dir:     filepath.Join(".claude", "skills", "rivet-promote-learnings"),
			file:    "SKILL.md",
			content: promoteLearningsSkill,
		},
	}
...
/rivet-promote-learnings instead.
...
5. After compacting, run ` + "`rivet sync`" + ` to update CLAUDE.md.
